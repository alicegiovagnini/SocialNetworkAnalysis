...
The analysis is carried out with NetworkX~\cite{networkx} on the simple, undirected and unweighted version of the network, and compared against an Erd\H{o}s--R\'enyi (ER) random graph and a Barab\'asi--Albert (BA) graph with (almost) the same number of nodes and edges. Static plots are produced in Python with Matplotlib, while the community-coloured network layout (Figure~\ref{fig:communities}) is produced with Gephi (ForceAtlas2).
...
Degree, betweenness (sampled), PageRank and eigenvector centrality largely agree on the top accounts (Table~\ref{tab:centrality}), the main exception being eigenvector centrality, which -- rewarding ties to other central nodes rather than raw popularity -- ranks the political hub \texttt{aoc} first and elevates journalism accounts such as \texttt{jamellebouie}. The platform account \texttt{bsky.app} tops the degree, betweenness and PageRank rankings, and overwhelmingly so for betweenness ($0.158$ vs $0.019$ for the runner-up), because almost every user follows it: it behaves as a global, non-thematic hub and could legitimately be treated as an outlier and removed. Once it is set aside and the centralities are recomputed, the ranking is led by the seed academic \texttt{carlbergstrom} and by political, journalism and outlet hubs (\texttt{aoc}, \texttt{edyong209}, \texttt{nytimes.com}, the astronomer \texttt{astrokatie.com}, \texttt{science.org}), confirming that beyond the platform account centrality is dominated by academics, science journalists and outlets. Notably, \texttt{science.org} climbs in the betweenness ranking, signalling a bridging role between otherwise separated thematic communities. The recurrence of political (\texttt{aoc}) and journalistic accounts among the most central nodes already hints at the overlap between science, journalism and politics on the platform.
...
Being a stochastic heuristic, Louvain was validated over 10 independent runs: it consistently returns $8$--$9$ communities with modularity $0.426\pm0.002$ (range $0.422$--$0.428$), and the partitions agree pairwise at $\mathrm{NMI}=0.85$. The modular structure is therefore stable across runs, so the partition adopted here (and released with the data) is representative rather than a lucky draw.
