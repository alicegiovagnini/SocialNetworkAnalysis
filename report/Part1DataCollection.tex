...
\begin{itemize}
    \item type diversity: to avoid starting from a single bubble, mixing organizations/media and individual communicators/aca\-demics.
    \item thematic centrality: preferring recognizable accounts that follow many members of the scientific community, so that the snowball fans out into several sub-communities. 
    \end{itemize}
    The selected seeds are \texttt{science.org} (Science Magazine), \texttt{scifri} (Science Friday), \texttt{standupforscience} (the ``Stand Up for Science'' movement), \texttt{whysharksmatter} (marine biologist D.~Shiffman) and \texttt{carlbergstrom} (academic C.~T.~Bergstrom): three outlets/organizations and two individual communicators. For reproducibility, it is worth noting that this endpoint returns a stable ranking (re-running the query reproduces the same 25 results, with the five seeds at ranks 6, 7, 11, 18 and 19), unlike the platform's interactive search bar, which relies on a different, personalised type-ahead endpoint and therefore yields a different ordering.
...
\begin{enumerate}\itemsep0pt
  ...
  \item every observed follow $u\!\to\!v$ is always written to disk; in addition, $v$ becomes a new node and is enqueued if and only if it is not already present and the node budget is not yet exhausted;
  \item $u$ is marked as processed and the loop continues until the queue is empty.
\end{enumerate}
Two parameters govern the process. The node budget is \texttt{MAX\_NODES}~$=15{,}000$: once reached, no new nodes are added, but the queue keeps being drained so that all edges internal to the collected set are still recorded. The per-node follow budget is capped at $10$ pages, i.e.\ at most 1{,}000 follows per node, to bound the cost on mega-hubs. The crawler (Python, \texttt{requests}) paces requests at $\sim$10\,req/s and automatically handles any transient request failures, staying within the platform's limits. Importantly, only outgoing follows are queried (\texttt{getFollows}); follower lists are never fetched, so an incoming edge to a node appears only when another collected node happens to follow it. The final network is the induced subgraph: from the raw edge dump only edges $u\!\to\!v$ whose endpoints are both among the 15{,}000 nodes are kept.
...
Identifiers are clean (no malformed or duplicate DIDs) and the core attributes have no missing values; only the optional display name is empty for 424 accounts (2.8\%). One potential issue regards post timestamps: since \texttt{createdAt} is declared by the publishing client, a few records carry implausible dates (the field ranges back to 1975, before the platform existed). These errors are filtered out before any temporal analysis.

\subsection{Choices and limitations}
Several deliberate choices shape and bound the sample.
\begin{itemize}
    \item Seed dependence: the sample characterizes the community around the seeds, not the whole platform, and the manual, heuristic seed choice influences the observed structure.
    ...
    \item Outgoing-only crawl: only follows are traversed, never followers, so in-degree reflects internal visibility, not true popularity. 
...
\end{itemize}
All results should therefore be read as characterizing the crawled scientific community, not the entire Bluesky platform.